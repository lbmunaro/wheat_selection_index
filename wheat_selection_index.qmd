...
```{r}
pheno_raw_l <- pheno_raw_l |>
  group_by(study_name, trait) |>
  filter(!is.na(mean(value, na.rm = TRUE))) |>
  filter(!location_name %in% c("Blacksburg, VA", "Warsaw, VA")) |>
  ungroup() |>
  glimpse()
```
...
pheno_raw_l |>
  ...
  geom_tile() +
  facet_wrap(~study_name+trait, scales = 'free')
```
...
mod.blup_single_trial <- function(dat) {
  mod <- asreml(value~1+block_number,
                random = ~idv(units) + germplasm_name,
                residual = ~ar1v(row_number):ar1(col_number),
                data=dat)
  # Update model
  mod <- update(mod)
  # Blups
  blups<- predict(mod, classify='germplasm_name',
                  ignore=c('(Intercept)','block_number'))$pvals
