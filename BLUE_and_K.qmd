...
```{r}
geno23 <- read.csv("data/geno2023.csv", header = T) |>
  rename(germplasm_name = X)
```
...
Subset blues with genotypic data
```{r}
blues_geno <- blues |>
  ungroup() |>
  inner_join(geno23|>dplyr::select(germplasm_name), by="germplasm_name") |>
  mutate_if(is.character, as.factor) |>
  glimpse()
```
Subset genotypic data
```{r}
geno23_filtered <- geno23 |>
  filter(germplasm_name %in% unique(blues_geno$germplasm_name) | grepl("IL2022", germplasm_name)) |>
  column_to_rownames("germplasm_name")
geno23_filtered |> tail(6)
```

Make relationship matrix
```{r}
geno23_filtered_mat <- as.matrix(geno23_filtered)

geno23_filtered_mat[1:5,1:5]
```
Relationship Matrix
```{r}
K <- rrBLUP::A.mat(geno23_filtered_mat)
K[1:5,1:5]
```
...
## Save workspace to open and run in the server
```{r}
objects()

rm(list=c("blues", "geno23", "geno23_filtered", "geno23_filtered_mat", "K",
          "pheno_raw_l", "pheno_raw_w", "reliability", "start.time", "time"))

save.image("data/blues_K.RData")
```